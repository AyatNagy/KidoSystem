...
import 'package:kido/services/audio_service.dart'; // استخدام الخدمة الخاصة بكِ
...
class _PuzzlePracticeScreenState extends State<PuzzlePracticeScreen>
  ...
  // 🔄 السيناريو المطلوب بالترتيب
  void _startFullFlow() async {
    if (!mounted) return;

    // 1️⃣ مرحلة الـ Intro: تظهر الصورة كاملة
    setState(() {
    ...
    // ننتظر مدة كافية حتى ينتهي الصوت (مثلاً 3 ثواني) قبل الانتقال
    await Future.delayed(const Duration(seconds: 3));

    if (!mounted) return;

    // 2️⃣ مرحلة الـ Modeling: شرح التركيب بالصوت والحركة
    setState(() {
      stage = PuzzleStage.modeling;
    });

    AudioService.play(fileName: "watch_me.mp3");
    // ننتظر ثانية قبل بدء حركة اليد لضمان استيعاب الطفل للصوت
    await Future.delayed(const Duration(seconds: 1));

    // تحريك اليد لكل القطع
    ...
    // 3️⃣ مرحلة الـ Interaction: الطفل يبدأ يسحب
    _startInteraction();
  }
  ...
  void _resetIdleTimer() {
    ...
    _idleTimer = Timer(const Duration(seconds: 5), () {
      ...
      // إظهار الهنت (اليد المساعدة والوميض)
      setState(() {
        showHint = true;
      });
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // 💡 الهنت والمساعدة (تظهر بعد 5 ثواني)
